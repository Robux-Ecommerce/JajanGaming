            ...
            <form action="{{ route('seller.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div style="margin-bottom: 1.5rem;">
                    ...
                    <textarea name="description" rows="5" required
                        style="width: 100%; background: rgba(15, 20, 30, 0.6); border: 1px solid rgba(100, 160, 180, 0.2); color: #e0e0e0; padding: 0.75rem 1rem; border-radius: 8px; font-size: 1rem; font-family: inherit;">{{ old('description') }}</textarea>
                </div>

                <div style="margin-bottom: 1.5rem;">
                    <label style="display: block; color: #b0b0b0; font-weight: 500; margin-bottom: 0.5rem;">Nama Game *</label>
                    <input type="text" name="game_name" value="{{ old('game_name') }}" required
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1.5rem; margin-bottom: 1.5rem;">
                    <div>
                        ...
                        <select name="game_type" required
                            ...
                            <option value="diamond" {{ old('game_type') == 'diamond' ? 'selected' : '' }}>Diamond</option>
                            <option value="coins" {{ old('game_type') == 'coins' ? 'selected' : '' }}>Coins</option>
                            <option value="gems" {{ old('game_type') == 'gems' ? 'selected' : '' }}>Gems</option>
                            <option value="robux" {{ old('game_type') == 'robux' ? 'selected' : '' }}>Robux</option>
                            <option value="voucher" {{ old('game_type') == 'voucher' ? 'selected' : '' }}>Voucher</option>
